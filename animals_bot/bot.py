...
logging.basicConfig(
    format="%(asctime)s [%(levelname)s]: %(message)s", level=CONFIG.LOGLEVEL.upper()
)
...
def main():
    ...
    while True:
        ...
        if type(last_seen_tweet_id) == bytes:
            last_seen_tweet_id = int(
                last_seen_tweet_id.decode("utf-8")
            )  # transform from bytes to string to, finally, integer
        else:
        ...
        for tweet in reversed(tweets):
            try:
                ...
                random_animal_message = (
                    RandomAnimalMessageBuilder.build_random_animal_message()
                )
                twitter_client.update_status(
                    f"@{tweet.user.screen_name} {random_animal_message}",
                    in_reply_to_status_id=tweet.id,
                )
            except BaseException:
                logging.exception(
                    f"Error while trying to reply to tweet ID {tweet.id}."
                )
            finally:
                redis_client.set("last_seen_tweet_id", tweet.id)
